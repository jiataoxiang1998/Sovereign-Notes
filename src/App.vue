...
<span class="font-['Manrope'] text-xs font-bold uppercase tracking-widest text-[#ffb4ab]">{{ currentLanguage === 'en' ? '04. Blockers' : '04. 无法解决的问题' }}</span>
        ...
        <template v-if="currentView === 'chat'">
          <div class="h-[calc(100vh-8rem)] flex flex-col">
            <div class="mb-6">
              <h1 class="text-4xl font-extrabold font-['Manrope'] tracking-tighter text-[#e5e2e1] mb-2">{{ currentLanguage === 'en' ? 'AI Chat' : 'AI 聊天' }}</h1>
              <p class="text-[#d0c5af]">{{ currentLanguage === 'en' ? 'Chat with AI assistant' : '与 AI 助手对话' }}</p>
            </div>
            ...
            <div class="flex-1 flex flex-col">
              ...
              <div class="flex gap-2">
                <input v-model="chatInput" @keyup.enter="sendChatMessage" :placeholder="currentLanguage === 'en' ? 'Type your message...' : '输入消息...'" class="flex-1 bg-[#2a2a2a] text-[#e5e2e1] px-4 py-3 rounded-lg border border-[#99907c]/30 focus:border-[#f2ca50] outline-none" :disabled="chatLoading" />
                <button @click="sendChatMessage" :disabled="chatLoading || !chatInput.trim()" class="px-6 bg-[#f2ca50] text-[#3c2f00] font-bold rounded-lg hover:brightness-110 transition" :class="{'opacity-50': chatLoading || !chatInput.trim()}">
...
const chatModelStatus = ref('') // '' | 'checking' | 'ready' | 'not-installed'
  ...
  if (chatModelStatus.value !== 'ready') {
    chatMessages.value.push({ 
      ...
      content: currentLanguage.value === 'zh' 
        ...
        : 'Please install Ollama and download the model first.\n1. Install: https://ollama.com\n2. Run: ollama pull qwen2.5\n3. Restart the app' 
    })
    return
  }
  ...
  try {
    ...
    fetch('http://localhost:11434/api/chat', {
      ...
      const reply = data.message?.content || 
        (isZh ? '无法生成回复' : 'Could not generate response')
      chatMessages.value.push({ role: 'assistant', content: reply })
    }).catch((error: any) => {
      console.error('Chat error:', error)
      chatMessages.value.push({ 
        role: 'assistant', 
        content: isZh ? `错误: ${error?.message}` : `Error: ${error?.message}` 
      })
    }).finally(() => {
    ...
    chatMessages.value.push({ 
      role: 'assistant', 
      content: currentLanguage.value === 'zh' ? `错误: ${error?.message}` : `Error: ${error?.message}` 
    })
    chatLoading.value = false
  }
}
